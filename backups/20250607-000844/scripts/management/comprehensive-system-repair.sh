...
fix_python_dependencies() {
    ...
    sudo apt update -qq || log "WARN" "Failed to update package lists"
...
fix_python_dependencies_on_pis() {
    ...
    if [[ -n "${PI_STATIC_IPS:-}" ]]; then
        ...
        # Try to discover Pis automatically
        print_step "Discovering Pi nodes..."
        mapfile -t pi_nodes < <(nmap -sn 192.168.3.0/24 | grep -E "192\.168\.3\.[0-9]+" | grep -oE "192\.168\.3\.[0-9]+" | head -5)
    fi
...
setup_pihole_dns() {
    ...
    # Get manager IP from discovered nodes or PI_STATIC_IPS
    local manager_ip=""
    if [[ -n "${PI_STATIC_IPS:-}" ]]; then
        # If PI_STATIC_IPS is a space-separated string, convert to array
        local pi_array=($PI_STATIC_IPS)
        manager_ip="${pi_array[0]}"
    else
        # Use first discovered node
        manager_ip="${discovered_nodes[0]}"
        print_warning "No PI_STATIC_IPS set, using discovered manager IP: $manager_ip"
    fi
    
    local domain_name="${PISWARM_DOMAIN:-piswarm.local}"
    
    print_step "Setting up Pi-hole DNS server on $manager_ip..."
    ...
    ssh "luser@$manager_ip" "
        # Add local domain entries to Pi-hole
        sudo tee -a /etc/pihole/custom.list > /dev/null << EOF
...
EOF
    ...
    local pi_nodes=()
    if [[ -n "${PI_STATIC_IPS:-}" ]]; then
        # Use PI_STATIC_IPS if available
        pi_nodes=($PI_STATIC_IPS)
    else
        # Use discovered nodes
        pi_nodes=("${discovered_nodes[@]}")
    fi
...
main() {
    ...
    setup_pihole_dns || log "WARN" "Pi-hole DNS setup had issues"
